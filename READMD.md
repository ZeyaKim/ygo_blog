...
- delteView를 구현할 때, confim 창 없이 바로 삭제되었는데, 딜리트뷰에서는 컨핌이 꼭 필요했기 때문에 이것은 다른 뷰를 사용해서 해결했습니다.

- 컨텍스트를 어떻게 전달하는지에 대한 이해가 부족했는데, 이 과정에서 많은 에러가 발생했습니다. 이것은 뷰에서 컨텍스트를 전달하는 방법을 이해하고, 템플릿에서 컨텍스트를 받아서 사용하는 방법을 이해하면서 해결했습니다.

- html에서 tags 기능을 사용해서 tag를 받을때, 문자열을 json화 하지 않고 사용하여 에러가 발생하였습니다. 이것은 json모듈을 사용해서 해결했습니다.

- 댓글 수정 시 댓글 창과 전환되는 로직을 구현할 때, html 숙련도가 부족해서 요소를 찾는데 어려움이 있었습니다. 이것은 GPT에 html을 전달하여 해결했습니다.
